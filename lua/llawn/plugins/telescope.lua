...
return {
  ...
  dependencies = {
    ...
    -- { dir = "~/Source/telescope-git.nvim", name = "telescope-git" },
  },
  config = function()
    ...
    -- Search/Grep
    vim.keymap.set("n", "<leader>f/", builtin.current_buffer_fuzzy_find, { desc = "Buffer Search" })
    vim.keymap.set("n", "<leader>fl", telescope.extensions.live_grep_args.live_grep_args, { desc = "Live Grep (Args)" })
    vim.keymap.set("n", "<leader>fw", builtin.grep_string, { desc = "Find word" })

    -- Neovim builtin
    vim.keymap.set("n", "<leader>fc", builtin.commands, { desc = "Nvim Commands" })
    vim.keymap.set("n", "<leader>fd", builtin.diagnostics, { desc = "LSP Diagnostics" })
    vim.keymap.set("n", "<leader>fh", builtin.help_tags, { desc = "Help Tags" })
    vim.keymap.set("n", "<leader>fk", builtin.keymaps, { desc = "Search Keymaps" })
    vim.keymap.set("n", "<leader>fo", builtin.oldfiles, { desc = "Old (Recent) Files" })
    ...
    vim.keymap.set("n", "<leader>fie", function()
      builtin.symbols({ sources = { "emoji" } })
    end, { desc = "Insert Emoji" })
    vim.keymap.set("n", "<leader>fig", function()
      builtin.symbols({ sources = { "gitmoji" } })
    end, { desc = "Insert Gitmoji" })
    vim.keymap.set("n", "<leader>fij", function()
      builtin.symbols({ sources = { "julia" } })
    end, { desc = "Insert Julia" })
    vim.keymap.set("n", "<leader>fik", function()
      builtin.symbols({ sources = { "kaomoji" } })
    end, { desc = "Insert Kaomoji" })
    vim.keymap.set("n", "<leader>fil", function()
      builtin.symbols({ sources = { "latex" } })
    end, { desc = "Insert LaTeX Symbols" })
    vim.keymap.set("n", "<leader>fim", function()
      builtin.symbols({ sources = { "math" } })
    end, { desc = "Insert Math Symbols" })
    vim.keymap.set("n", "<leader>fin", function()
      builtin.symbols({ sources = { "nerd" } })
    end, { desc = "Insert Nerd Fonts" })
    ...
    -- Others
    vim.keymap.set("n", "<leader>ft", telescope.extensions.undo.undo, { desc = "Undo Tree" })
  end,
}
